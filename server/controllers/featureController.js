...
// ==================== AI PLANNER ====================
exports.getAIRecommendations = catchAsync(async (req, res, next) => {
  const { city, budget, guestCount, weddingType, brideName, groomName, weddingDate, traditions } = req.body;

  if (!city || !budget || !guestCount) {
    return next(new AppError('Please provide city, budget and guest count', 400));
  }

  // Dynamic budget allocation based on Wedding Type
  let budgetAllocation = {
    'Venues & Catering': budget * 0.45,
    'Decoration': budget * 0.15,
    'Photography': budget * 0.10,
    'Makeup & Attire': budget * 0.10,
    'Entertainment': budget * 0.08,
    'Transport': budget * 0.07,
    'Other': budget * 0.05
  };

  if (weddingType === 'Destination') {
    budgetAllocation = {
      'Venues & Catering': budget * 0.35,
      'Transport & Logistics': budget * 0.20,
      'Accommodation': budget * 0.15,
      'Decoration': budget * 0.10,
      'Photography': budget * 0.10,
      'Makeup & Attire': budget * 0.05,
      'Other': budget * 0.05
    };
  } else if (weddingType === 'Minimalist') {
    budgetAllocation = {
      'Venues & Catering': budget * 0.50,
      'Decoration': budget * 0.05,
      'Photography': budget * 0.20,
      'Makeup & Attire': budget * 0.10,
  ...
  for (const cat of categories) {
    // Try to find vendors within budget first
    let vendors = await Vendor.find({
      'location.city': { $regex: new RegExp(city, 'i') },
      category: cat._id,
      approvalStatus: process.env.NODE_ENV === 'development' ? { $in: ['approved', 'pending'] } : 'approved',
      basePrice: { $lte: budgetAllocation[cat.name] || budgetAllocation['Other'] || budget * 0.2 }
    })
      .sort({ 'rating.average': -1 })
      .limit(2);

    // Fallback: If no vendors found in budget, find top rated vendors in that city regardless of price
    if (vendors.length === 0) {
  ...
  const wDate = weddingDate ? new Date(weddingDate) : new Date(Date.now() + 180 * 24 * 60 * 60 * 1000);
  const now = new Date();
  const monthsAway = Math.max(1, Math.round((wDate - now) / (1000 * 60 * 60 * 24 * 30)));

  const timeline = [];
  if (monthsAway > 6) {
    timeline.push({ task: 'Set Wedding Date & Venue', time: `${monthsAway - 2} months before` });
    timeline.push({ task: 'Book Core Vendors (Caterer, Photographer)', time: `${monthsAway - 4} months before` });
    timeline.push({ task: 'Shopping & Outfits', time: '6-5 months before' });
  } else {
    timeline.push({ task: 'Rush Booking: Venue & Vendors', time: 'Immediate Action Required' });
    timeline.push({ task: 'Shopping & Outfits', time: 'Within next 2 weeks' });
  }

  timeline.push({ task: 'Send Save-the-Dates / Invites', time: '2-3 months before' });
  timeline.push({ task: 'Finalize Guest List & Logistics', time: '1 month before' });
  timeline.push({ task: `Celebrate ${brideName || 'Bride'} & ${groomName || 'Groom'}'s Day!`, time: wDate.toLocaleDateString() });
  ...
  res.status(200).json({
    ...
    data: {
      ...
      recommendations,
      timeline,
      meta: { brideName, groomName, weddingDate, traditions, weddingType }
    }
  });
});
